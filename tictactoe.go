...
func (e *markAlreadyExistError) Error() string {
    return fmt.Sprintf("postion (%d, %d) already has a mark on it.", e.row, e.column)
} 

func (e *positionOutOfBoundError) Error() string {
    return fmt.Sprintf("postion (%d, %d) is out of bound.", e.row, e.column)
} 
...
func (state *gameState) checkForWinner() gameResult {
    boardSize := len(state.board) // assume the board is square
    ...
    checkLine := func(startRow int, startColumn int, deltaRow int, deltaColumn int) gameResult {
        ...
        // loop starts from the second square
        for row >= 0 && column >= 0 && row < boardSize && column < boardSize {
            
                // there can't be a winner if an empty square is present within the line
                if state.board[row][column] == none {
                    return noWinnerYet
                } 
...
func main() {
        state := gameState{}
        state.turnPlayer = cross // cross is 1st
        ...
        for {
                ...
                for {
                        ...
                        // if a valid position was entered, break out from the loop
                        if e == nil {
                                break
                        }
                        
                        // if an invalid position was entered, prompt the playeer to re-enter another position
                        fmt.Println(e)
